...
CREATE WAREHOUSE IF NOT EXISTS my_warehouse
  WITH
  WAREHOUSE_SIZE = 'X-SMALL'  -- Options: X-SMALL, SMALL, MEDIUM, LARGE, X-LARGE, etc.
  AUTO_SUSPEND = 60            -- Auto-pause after 60 seconds of inactivity (saves money!)
  AUTO_RESUME = TRUE           -- Auto-start when a query runs
  INITIALLY_SUSPENDED = TRUE;  -- Start in suspended state (saves money!)

USE WAREHOUSE my_warehouse;

CREATE DATABASE IF NOT EXISTS customer_surveys
  COMMENT = 'Database for customer survey data from S3';

USE DATABASE customer_surveys;

CREATE SCHEMA IF NOT EXISTS customer_surveys.raw_data
  COMMENT = 'Raw data from S3 - no transformations';

USE SCHEMA customer_surveys.raw_data;

CREATE SCHEMA IF NOT EXISTS customer_surveys.public
  COMMENT = 'public from S3 - no transformations';
...
CREATE OR REPLACE STORAGE INTEGRATION integration_of_surveys
  TYPE = EXTERNAL_STAGE
  STORAGE_PROVIDER = 'S3'
  ENABLED = TRUE
  STORAGE_AWS_ROLE_ARN = 'arn:aws:iam::463734035383:role/mysnowflakerole_forcustomersurveys'
  STORAGE_ALLOWED_LOCATIONS = ('s3://customer-survey-bucket-dp2-v1/')
  STORAGE_BLOCKED_LOCATIONS = ('s3://jacks-bucket-dp1/');

  DESC INTEGRATION integration_of_surveys;

  CREATE ROLE IF NOT EXISTS mysnowflakerole_forcustomersurveys
  COMMENT = 'Role for customer survey data operations';
...
CREATE FILE FORMAT IF NOT EXISTS customer_surveys.raw_data.survey_csv_format
  TYPE = 'CSV'
  FIELD_DELIMITER = ','
  SKIP_HEADER = 1
...
CREATE STAGE my_s3_stage
  STORAGE_INTEGRATION = integration_of_surveys
  URL = 's3://customer-survey-bucket-dp2-v1/'
...
SELECT $1, $2, $3, $4, $5
FROM @customer_surveys.raw_data.my_s3_stage
(FILE_FORMAT => customer_surveys.raw_data.survey_csv_format)
LIMIT 10;
